...
@section('title', 'Crear Suministro')

@section('content')
<div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg">
    <h1 class="text-4xl font-bold text-gray-800 dark:text-gray-100 mb-6">Crear Suministro</h1>

    <form action="{{ route('supplies.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="number" name="amount" id="amount" value="{{ old('amount') }}" required step="0.01"
                class="w-full p-3 border rounded-md dark:bg-gray-700 dark:text-gray-200 focus:ring focus:ring-green-500">
        </div>
